...
;!function(){
  ...
  */
  ...
  Class.prototype.done = function(param, type){
    ...
    typeof options[type || 'done'] === 'function' && options[type || 'done'].apply(that, param);
  ...
  Class.prototype.events = function(){
    ...
    ,showEvent = function(elem, bind){
      elem.on(options.trigger, function(){
        ...
        that.index == this.getAttribute('lay-key') && that.render();
      });
    };
    ...
    lay(document).on('click', function(e){
      ...
      // 已过时的就不需要处理了
      that.index == options.elem[0].getAttribute('lay-key') && that.remove();
